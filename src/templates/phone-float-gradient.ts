...
/**
 * PHONE-FLOAT-GRADIENT — celular centralizado com fundo BADGE_GRADIENT vibrante.
 * Cards: card-bubble fundo branco sólido, demais cards com dark glass.
 * Accent no headline é branco itálico (contrasta com fundo colorido).
 * CTA é botão branco com texto escuro.
 */
export function buildPhoneFloatGradient(variant: CopyVariant, imageSrc: string, _styleConfig: StyleConfig, isStory = false): string {
...
.bg{position:absolute;inset:0;background:${BADGE_GRADIENT};}
.floor-glow{position:absolute;bottom:155px;left:50%;transform:translateX(-50%);width:320px;height:40px;background:radial-gradient(ellipse,rgba(255,255,255,0.3) 0%,transparent 70%);filter:blur(8px);}

.phone-wrap{position:absolute;top:28px;left:50%;transform:translateX(-50%);width:210px;height:420px;}
.phone{position:absolute;inset:0;border-radius:36px;background:linear-gradient(160deg,#2a2a3a 0%,#111118 40%,#0a0a10 100%);box-shadow:0 0 0 1.5px rgba(255,255,255,0.2), 0 0 0 3px rgba(0,0,0,0.3), 0 30px 80px rgba(0,0,0,0.4), 0 0 60px rgba(255,255,255,0.15);}
...
.card-bubble{position:absolute;top:110px;left:-108px;width:120px;background:#ffffff;border-radius:14px;padding:10px 12px;box-shadow:0 6px 24px rgba(0,0,0,0.2);transform:rotate(-4deg);z-index:20;}
...
.card-xp{position:absolute;top:90px;right:-112px;width:118px;background:rgba(10,8,20,0.82);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.12);border-radius:14px;padding:10px 12px;box-shadow:0 6px 24px rgba(0,0,0,0.3);transform:rotate(3deg);z-index:20;}
.card-xp-label{font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:rgba(255,255,255,0.45);margin-bottom:5px;}
...
.card-scene{position:absolute;bottom:120px;left:-96px;width:104px;background:rgba(10,8,20,0.82);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.12);border-radius:12px;padding:9px 11px;box-shadow:0 6px 24px rgba(0,0,0,0.3);transform:rotate(2deg);z-index:20;}
...
.card-scene-label{font-size:10px;font-weight:700;color:rgba(255,255,255,0.45);text-transform:uppercase;letter-spacing:.07em;margin-bottom:3px;}
.card-scene-val{font-size:11px;font-weight:800;color:#fff;}

.card-streak{position:absolute;bottom:105px;right:-106px;width:110px;background:rgba(10,8,20,0.82);backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.12);border-radius:12px;padding:9px 12px;box-shadow:0 6px 24px rgba(0,0,0,0.3);transform:rotate(-3deg);z-index:20;}
...
h1{font-size:${h1Sz}px;font-weight:900;color:#ffffff;line-height:1.04;letter-spacing:-.025em;margin-bottom:14px;}
.accent{color:#ffffff;font-style:italic;}
.cta-row{display:flex;align-items:center;justify-content:center;gap:14px;}
.cta-btn{background:#ffffff;border-radius:100px;padding:11px 26px;font-size:14px;font-weight:800;color:#1a1030;}
.cta-free{font-size:12px;font-weight:600;color:rgba(255,255,255,.65);}
</style></head><body>
<div class="post">
  <div class="bg"></div>
