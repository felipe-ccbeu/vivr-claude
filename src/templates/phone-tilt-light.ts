...
/**
 * PHONE-TILT-LIGHT — variante clara do phone-tilt.
 * Fundo #f5f4f8, copy em tons escuros, phone idêntico ao dark.
 */
export function buildPhoneTiltLight(variant: CopyVariant, imageSrc: string, _styleConfig: StyleConfig, isStory = false): string {
  const headlineHtml = highlightAccentWord(variant.headline, variant.accentWord)
  const H = isStory ? STORY_HEIGHT : 675
  const copyPadTop = isStory ? 80 : 48
  const copyPadBottom = isStory ? STORY_SAFE_BOTTOM : 48
  const h1Sz = isStory ? 36 : 32
...
.bg-accent{position:absolute;inset:0;background:radial-gradient(ellipse at 75% 85%,rgba(38,182,205,0.08) 0%,transparent 50%);}
.bg-accent2{position:absolute;inset:0;background:radial-gradient(ellipse at 10% 75%,rgba(233,72,153,0.06) 0%,transparent 45%);}
...
/* LEFT COPY BLOCK */
.copy-block{position:absolute;top:0;left:0;width:262px;height:100%;display:flex;flex-direction:column;justify-content:center;padding:${copyPadTop}px 0 ${copyPadBottom}px 36px;z-index:10;}
.tag{display:inline-flex;align-items:center;gap:6px;background:rgba(0,0,0,0.04);border:1px solid rgba(0,0,0,0.1);border-radius:100px;padding:5px 12px;margin-bottom:18px;width:fit-content;}
.tag-dot{width:6px;height:6px;border-radius:50%;background:linear-gradient(135deg,#f97040,#e94899);}
.tag-text{font-size:10px;font-weight:700;letter-spacing:.1em;text-transform:uppercase;color:rgba(0,0,0,0.45);}
h1{font-size:${h1Sz}px;font-weight:900;color:#1a1030;line-height:1.05;letter-spacing:-.02em;margin-bottom:16px;}
...
.cta-btn{display:inline-block;background:linear-gradient(135deg,#f7c948 0%,#f97040 20%,#e94899 45%,#9b5de5 65%,#26c6da 83%,#80e27e 100%);border-radius:100px;padding:13px 28px;font-size:14px;font-weight:800;color:#fff;margin-bottom:10px;box-shadow:0 4px 16px rgba(233,72,153,0.3);margin-top:20px;}
.cta-free{font-size:11px;font-weight:600;color:rgba(0,0,0,0.3);}

/* PHONE */
.phone-wrap{position:absolute;top:60px;right:-30px;width:260px;height:500px;transform:rotate(10deg);z-index:15;}
...
/* FLOATING CARDS */
.card-xp{position:absolute;top:30px;left:-60px;width:114px;background:linear-gradient(135deg,rgba(38,182,205,0.2),rgba(131,120,182,0.25));backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.15);border-radius:14px;padding:11px 13px;box-shadow:0 8px 28px rgba(0,0,0,0.4);transform:rotate(-5deg);z-index:20;}
.card-xp-label{font-size:9px;font-weight:700;letter-spacing:.08em;text-transform:uppercase;color:rgba(255,255,255,0.5);margin-bottom:5px;}
...
.card-xp-val{font-size:13px;font-weight:800;color:#fff;}

.card-streak{position:absolute;bottom:-40px;left:170px;width:108px;background:linear-gradient(135deg,rgba(249,112,64,0.25),rgba(233,72,153,0.2));backdrop-filter:blur(12px);border:1px solid rgba(255,255,255,0.12);border-radius:14px;padding:11px 13px;box-shadow:0 8px 28px rgba(0,0,0,0.4);transform:rotate(4deg);z-index:20;}
.card-streak-emoji{font-size:18px;line-height:1;margin-bottom:4px;}
.card-streak-label{font-size:9px;font-weight:700;color:rgba(255,255,255,0.45);text-transform:uppercase;letter-spacing:.07em;}
.card-streak-val{font-size:15px;font-weight:900;color:#fff;}

/* DIVIDER LINE */
.divider{position:absolute;top:60px;bottom:60px;left:250px;width:1px;background:linear-gradient(to bottom,transparent,rgba(0,0,0,0.08) 30%,rgba(0,0,0,0.08) 70%,transparent);z-index:5;}
</style></head><body>
<div class="post">
  <div class="bg"></div>
